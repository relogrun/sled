...
use sled_core::{Call, Slot, ToolExecutor};
...
impl ToolRegistry {
    ...
    pub async fn execute(&self, ctx: &ToolContext, call: &Call) -> Result<Value> {
        info!(tool = %call.tool, "executing tool");
        let Some(tool) = self.tools.get(&call.tool) else {
            warn!(tool = %call.tool, "unknown tool requested");
            return Ok(json!({"ok": false, "error": format!("unknown tool: {}", call.tool)}));
        };
